...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| [interp-engine eager](eager.json) *(reference)* | ref | ok | float16 | v1.1.0+dirty | — | — | — | — |
| [interp-engine vllm](vllm.json) | ✅ | ok | float16 | v0.26.0 | 26 | 0 | 0 | 3 |
| [interp-engine vllm-static](vllm-static.json) | ✅ | ok | float16 | v0.27.1 | 24 | 0 | 0 | 0 |
| [tlens_v2](tlens_v2.json) | ✅ | ok | float16 | v3.7.0 | 21 | 0 | 0 | 3 |
| [tlens_v3](tlens_v3.json) | ✅ | ok | float16 | v3.7.0 | 9 | 0 | 0 | 15 |
...
| `mlp_out_post`<br>layer 6 | ref | ✅ | ✅ | ✅ | n/a | — |
| `mlp_out_post`<br>layer 11 | ref | ✅ | ✅ | ✅ | n/a | — |
...
| `mlp_pre`<br>layer 6 | ref | — | — | ✅ | n/a | ✅ |
| `mlp_pre`<br>layer 11 | ref | — | — | ✅ | n/a | ✅ |
| `mlp_act`<br>layer 0 | ref | ✅ | ✅ | ✅ | n/a | ✅ |
| `mlp_act`<br>layer 6 | ref | ✅ | ✅ | ✅ | n/a | ✅ |
| `mlp_act`<br>layer 11 | ref | ✅ | ✅ | ✅ | n/a | ✅ |
| `embeddings` | ref | ✅ | — | — | — | — |
| `final_norm` | ref | ✅ | — | — | — | — |
...
| --- | --- | --- | --- |
| `resid_mid` | 0, 6, 11 | nnsight, interp-engine vllm | neither engine captured it |
...
| `mlp_out_post` | 0, 6, 11 | tlens_v3 | this engine declined the point — OPT inlines `fc1`/`fc2` on the decoder layer, so the bridge's `mlp` component wraps nothing that runs: `blocks.N.hook_mlp_out` is registered and never fires (checked directly on opt-125m). Its attention hooks do fire, which is why the rest of the cell scores |
| `mlp_pre` | 0, 6, 11 | tlens_v3 | this engine declined the point — OPT inlines `fc1`/`fc2` on the decoder layer, so the bridge's `mlp` component wraps nothing that runs: `blocks.N.hook_mlp_out` is registered and never fires (checked directly on opt-125m). Its attention hooks do fire, which is why the rest of the cell scores |
| `mlp_act` | 0, 6, 11 | tlens_v3 | this engine declined the point — OPT inlines `fc1`/`fc2` on the decoder layer, so the bridge's `mlp` component wraps nothing that runs: `blocks.N.hook_mlp_out` is registered and never fires (checked directly on opt-125m). Its attention hooks do fire, which is why the rest of the cell scores |
