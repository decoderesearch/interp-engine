...
def _make_steer_modifier(spec: dict, dev, dt):
    ...
    ``op`` is a :class:`~interp_engine.steer_specs.SteerMethod`. ``additive`` -> constant
...
def worker_install_steering(worker: object, specs: list[dict]) -> None:
    ...
    "op": a SteerMethod value, "vector": list[float], and op args ("coeff" for
    additive and orthogonal; "min"/"max" for projection_cap)}``. Applied to all token positions.
