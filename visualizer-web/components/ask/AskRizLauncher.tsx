 ...
 * the icon hamburger — so it becomes the intercom convention instead: a circle
 * pinned over the bottom right corner. The mode toggle gets the same treatment
...
export function AskRizLauncher({ state }: { state: VisualizerState }) {
  ...
  return (
    <>
      ...
      {hydrated &&
        createPortal(
          <button
            ...
            className={`fixed right-4 bottom-4 z-50 flex h-12 w-12 cursor-pointer items-center justify-center rounded-full border shadow-lg shadow-slate-900/20 transition-colors sm:hidden ${
              open
      ...
      <AskRizPanel id={PANEL_ID} open={open} onClose={close} state={state} />
    </>
  );
}
