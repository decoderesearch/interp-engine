...
/**
 * Nothing but the link, which is why it is a component of its own.
 *
 * Keyed on the query rather than threading it into a setter. The query cannot be
 * read during the prerender, nor during the hydration render that has to match
 * it, so it arrives one render late — and applying it from there would be a
 * setState in an effect body, which `lib/useHydrated.ts` documents as the thing
 * this app writes around. A remount takes the link as an initial condition
 * instead, and only ever happens when there is one: a plain visit reads `/` both
 * times and keeps the tree it hydrated.
 */
export default function Page() {
  const link = useOpenedWith();
  return <Visualizer key={`/${encodeLink(link)}`} link={link} />;
}

function Visualizer({ link }: { link: Link }) {
  ...
  return (
    <main className="flex h-dvh flex-col overflow-hidden">
      ...
      <header className="relative z-30 grid shrink-0 grid-cols-[1fr_auto] items-center gap-x-3 border-b border-slate-200 bg-white/85 px-3 py-2 backdrop-blur sm:grid-cols-[1fr_auto_1fr]">
        <div className="flex min-w-0 items-center gap-x-3">
          ...
          <div className="min-w-0">
            ...
            <div className="font-mono text-sm font-semibold whitespace-nowrap text-slate-800">
              interp-engine
            </div>
        ...
        <div className="hidden justify-self-center sm:block">
          <ModeToggle
            ...
            onSingle={() => state.setCompareId(null)}
            onCompare={state.startCompare}
          />
        </div>
